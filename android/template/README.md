...
`docs/mobile-hosted-backend.md`). It does **not** depend on `moui_shell` or the
embedding inject ABI.

## Layout

- `app/` — Gradle application module (placeholder Activity)
- Native bootstrap symbols live under `android/native/` and be linked
  into the app shared library by the app build

## Status

- **M1**: host-sim coverage lives in `moon test -p android` (no device required)
...
```sh
bash scripts/check_android_hosted_smoke.sh
```
...
This template lives under `window/android/template` next to native glue and
