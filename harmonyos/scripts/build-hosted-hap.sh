...
WINDOW_ROOT="$(cd "$PKG_ROOT/.." && pwd)"
WORKSPACE="${MBW_WORKSPACE_ROOT:-}"
if [[ -z "$WORKSPACE" ]]; then
  if [[ -d "$WINDOW_ROOT/../examples/counter/harmonyos_window_hosted" ]]; then
    WORKSPACE="$(cd "$WINDOW_ROOT/.." && pwd)"
  else
    WORKSPACE="$WINDOW_ROOT"
  fi
fi
...
bash "$WINDOW_ROOT/scripts/check_harmonyos_hosted_smoke.sh"
...
if [[ -x "$HVIGORW" && -x "$OHPM" && -d "$DEVECO_SDK_HOME" && -f "$SDK_HOME/native/build/cmake/ohos.toolchain.cmake" ]]; then
  ...
  export MBW_WINDOW_ROOT="$WINDOW_ROOT"
